...
import { Currency, CurrencyAmount, WDOGE, JSBI, Token, TokenAmount } from '@huskyswap/sdk'
...
export function useCurrencyBalances(
  ...
  const containsETH: boolean = useMemo(() => currencies?.some(currency => currency === WDOGE) ?? false, [currencies])
  ...
  return useMemo(
    () =>
      currencies?.map(currency => {
        ...
        if (currency === WDOGE) return ethBalance[account]
        return undefined
      }) ?? [],
